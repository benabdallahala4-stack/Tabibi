    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      {appointments === null ? (
        ...
        <div className="mt-8 rounded-2xl bg-white p-10 text-center ring-1 ring-slate-200">
          <p className="text-slate-600">{t("mine.none")}</p>
          <Link
            href="/recherche"
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    ...
                    {a.kind === "cabinet" ? t("booking.inCabinet") : t("booking.inTele")}
                  </p>
                  <p className="mt-1 text-sm font-medium text-primary-700" dir="ltr">
                    {a.dateIso} {a.time}
                  </p>
                </div>
                <div className="flex flex-col gap-2 sm:items-end">
                  {a.kind === "teleconsultation" && (
                    <Link
                      href={`/visio?rdv=${a.id}`}
                      className="rounded-xl bg-sky-50 px-4 py-2 text-center text-sm font-medium text-sky-700 transition hover:bg-sky-100"
                    >
                      {t("mine.join")}
                    </Link>
                  )}
                ...
                {cancelled.map((a) => (
                  ...
                  >
                    <span className="line-through">
                      {a.doctorName} — <span dir="ltr">{a.dateIso} {a.time}</span>
                    </span>{" "}
                    <span className="mx-2 rounded-full bg-red-50 px-2 py-0.5 text-xs text-accent-600">
                      {t("mine.cancelledBadge")}
                    </span>
                  </div>
                ))}
              </div>
